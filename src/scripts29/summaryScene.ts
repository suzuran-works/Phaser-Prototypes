...
type Creature = {
  ...
  toolLabel: Phaser.GameObjects.Text;
...
class SummaryScene extends BaseResponsiveScene {
  public static readonly key = 'Scripts29SummaryScene';
  ...
  public update(_: number, deltaMs: number): void {
    ...
    this.creatures.forEach((creature) => {
      ...
      creature.toolLabel.setPosition(iso.x, iso.y - this.tileSize * 0.96);
      creature.toolLabel.setText(creature.carryingTool ?? "");
      creature.info.setPosition(iso.x, iso.y - this.tileSize * 0.42);
      creature.info.setText(creature.mood);
    });
  ...
  protected renderLayout(layout: QuarterLayout): void {
    ...
    this.reels.forEach((reel, index) => {
      ...
      reel.frame.setPosition(x, y).setSize(Math.max(68, Math.floor(layout.width * 0.1)), Math.max(88, Math.floor(layout.slotAreaHeight * 0.28)));
      reel.text.setPosition(x, y).setFontSize(`${Math.max(36, Math.floor(layout.width * 0.045))}px`);
    });
  ...
  private spawnCreature(emoji: string): void {
    ...
    const toolLabel = this.add.text(iso.x, iso.y - this.tileSize * 0.96, '', {
      fontFamily: 'sans-serif',
      fontSize: `${Math.max(18, Math.floor(this.tileSize * 0.55))}px`,
    }).setOrigin(0.5, 0.5);
    ...
    this.creatureLayer.add([shadow, sprite, toolLabel, info]);
  ...
  private refreshDroppedItems(): void {
    ...
    this.creatures.forEach((creature) => {
      ...
      creature.toolLabel.setFontSize(`${Math.max(18, Math.floor(this.tileSize * 0.55))}px`);
